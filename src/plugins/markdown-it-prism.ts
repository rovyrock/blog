...
const LANGS = ["js", "bin", "css", "html", "python"];
 ...
 */
function loadPrismLang(lang: string = "js"): Grammar | undefined {
  if (!lang || !LANGS.includes(lang)) return undefined;
  ...
  if (langObject === undefined) {
    ...
    try {
      //const langObjectRequire = require(`prismjs/components/prism-${lang}.min.js`);
      //console.log("loadPrismLang=======", lang, langObjectRequire);
      //loadPrismLang(language)
      loadLanguages(lang);
      loadLanguages([lang]);
      langObject = Prism.languages[lang];
    } catch (e) {
      throw new Error(
        `Cannot load Prism components "${lang}". Please check the spelling.`
      );
    }
  }
  return langObject;
